...
static MMDB_lookup_result_s lookup_or_die(MMDB_s *mmdb, const char *ip_address);
...
int main(int argc, char **argv) {
  ...
  MMDB_lookup_result_s lookup_result = lookup_or_die(&mmdb, ip_address);

  // Print the country
  int exit_code = print_country(&lookup_result, ip_address);
...
static void load_arguments(int argc, char **argv, char **mmdb_filename, char **ip_address) {
  ...
  if (help) {
    usage(program_name, 0, NULL);
  }

  if (*mmdb_filename == NULL) {
    usage(program_name, 1, "You must profile a MaxMind DB file with --file");
  }

  if (*ip_address == NULL) {
    usage(program_name, 1, "You must provide an IP address with --ip");
  }  
}
